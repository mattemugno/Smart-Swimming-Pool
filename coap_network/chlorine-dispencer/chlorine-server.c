...
#define ACTUATOR_TYPE "chlorine_dispenser"
...
extern coap_resource_t res_chlorine_switch;
...
PROCESS(chlorine_server, "Smart swimming pool Chlorine Dispenser Server");
AUTOSTART_PROCESSES(&chlorine_server);
...
PROCESS_THREAD(chlorine_server, ev, data)
{
  PROCESS_BEGIN();

  LOG_INFO("Smart swimming pool Chlorine Dispenser COAP Server\n");
  ...
  LOG_INFO("Starting Chlorine Dispenser CoAP-Server\n");
  coap_activate_resource(&res_chlorine_switch, "chlorine-dispenser/switch"); 
