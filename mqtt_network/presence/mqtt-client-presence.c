...
/*---------------------------------------------------------------------------*/
#define LOG_MODULE "mqtt-client-presence"
...
/*---------------------------------------------------------------------------*/
PROCESS_NAME(mqtt_client_presence);
AUTOSTART_PROCESSES(&mqtt_client_presence);
...
/*---------------------------------------------------------------------------*/
PROCESS(mqtt_client_presence, "MQTT Client - Presence");
...
static void pub_handler(const char *topic, uint16_t topic_len, const uint8_t *chunk, uint16_t chunk_len)
{
	...
	if(strcmp(topic, "presence-command") != 0){
	...
	if (strcmp((const char*) chunk, "ON") == 0){
		LOG_INFO("Switch ON ligths \n");
		presence = true;	
	}else if (strcmp((const char*) chunk, "OFF") == 0){
		LOG_INFO("Switch OFF lights \n");	
		presence = false;
	} 
}
...
static void mqtt_event(struct mqtt_connection *m, mqtt_event_t event, void *data)
{
  switch (event) {
    ...
    case MQTT_EVENT_DISCONNECTED:
      ...
      process_poll(&mqtt_client_presence);
      break;
...
PROCESS_THREAD(mqtt_client_presence, ev, data)
{
  PROCESS_BEGIN();

  printf("MQTT Client Process - Presence\n");
  ...
  /* Broker registration */ 
  mqtt_register(&conn, &mqtt_client_presence, client_id, mqtt_event,
  ...
  while (1) {
	...
	status = mqtt_subscribe(&conn, NULL, "presence-command", MQTT_QOS_LEVEL_0);
	...
	state = STATE_SUBSCRIBED;
      ...
      if (state == STATE_SUBSCRIBED) {
        /* Publish a message on topic "presence"*/
        sprintf(pub_topic, "%s", "presence");
	set_presence();
        sprintf(app_buffer, "{\"nodeId\": %d, \"presence\": %s}", node_id, btoa(presence));
